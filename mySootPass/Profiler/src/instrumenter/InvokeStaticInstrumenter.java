 ...
 * instrumenter.InvokeStaticInstrumenter inserts count instructions before
...
/* instrumenter.InvokeStaticInstrumenter extends the abstract class BodyTransformer,
...
public class InvokeStaticInstrumenter extends BodyTransformer {
	...
	static {
		counterClass = Scene.v().loadClassAndSupport("instrumenter.MyCounter");
	...
	protected void internalTransform(Body body, String phase, Map options) {
		...
		while (stmtIt.hasNext()) {
			...
			//System.out.println("Print stmt: " + stmt.toString());
		...
		if (isMain) {
			...
			while (stmtIt.hasNext()) {
				...
				if ((stmt instanceof ReturnStmt)
					...
					// 1. make invoke expression of instrumenter.MyCounter.report()
